...
std::string Class::GetName() {
  uint32_t index = 0;
  for (uint32_t i = m_Name.size() - 1; i >= 0; i--) {
    if (m_Name[i] == ':') {
      index = i + 1;
      break;
    }
...
ReflectionData GetReflectionDataFromFolder(const fs::path &path) {
  ...
  for (auto &p : fs::directory_iterator("generated")) {
    ...
    while (getline(ifs, line)) {
      ...
      if (tokens[0] == "include") {
        if (!hasEntry(data.m_Headers, tokens[1]))
          data.m_Headers.push_back(tokens[1]);
      }
      if (tokens[0] == "class") {
        std::string name = tokens[1];
        std::string attrib = tokens[2];
